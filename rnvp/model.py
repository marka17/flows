...
class RealNVP(nn.Module):
    ...
    def g(self, x: torch.Tensor) -> torch.Tensor:
        for mask, s, t in zip(self.masks, self._scale_networks, self._translation_networks):
            ...
            x = (x * torch.exp(scaled_x) + translated_x) * (1 - mask) + masked_x
    ...
    def f(self, x: torch.Tensor) -> torch.Tensor:
        ...
        triplets = reversed(list(zip(self.masks, self._scale_networks, self._translation_networks)))
        for mask, s, t in triplets:
            masked_z = z * mask

            scaled_z = s(masked_z)
            translated_z = t(masked_z)

            z = (torch.exp(-scaled_z) * (z - translated_z)) * (1 - mask) + masked_z
